...
/**
 * @param RC The render context, can be any object that holds what's required for rendering.
 *          See [Node.render]
 * @param T The type of nodes that are handled.
 * @param S The state of the current parser.
 */
open class Parser<RC, T : Node<RC>, S> @JvmOverloads constructor(private val enableDebugging: Boolean = false) {

    private val rules = ArrayList<Rule<RC, out T, S>>()

    @Suppress("unused")
    fun addRule(rule: Rule<RC, out T, S>) =
        this.apply { rules.add(rule) }

    fun addRules(vararg newRules: Rule<RC, out T, S>) =
        this.addRules(newRules.asList())

    fun addRules(newRules: Collection<Rule<RC, out T, S>>) =
    ...
    private fun <RC, T : Node<RC>, S> logMatch(rule: Rule<RC, T, S>, source: CharSequence) {
        if (enableDebugging) {
            println(
                "$TAG: MATCH: with rule with pattern: " + rule.matcher.pattern()
                    .toString() + " to source: " + source
            )
        }
    }

    private fun <RC, T : Node<RC>, S> logMiss(rule: Rule<RC, T, S>, source: CharSequence) {
        if (enableDebugging) {
            println(
                "$TAG: MISS: with rule with pattern: " + rule.matcher.pattern()
                    .toString() + " to source: " + source
            )
        }
    }
