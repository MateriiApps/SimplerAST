...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        parser = Parser<RenderContext, Node<RenderContext>, ParseState>()
            ...
            .addRules(
                CustomMarkdownRules.createMarkdownRules(
                    this@MainActivity,
                    listOf(R.style.Demo_Header_1, R.style.Demo_Header_2, R.style.Demo_Header_3),
                    listOf(
                        R.style.Demo_Header_1_Add,
                        R.style.Demo_Header_1_Remove,
                        R.style.Demo_Header_1_Fix
                    )
                )
            )
            .addRules(
    ...
    data class ParseState(override val isInQuote: Boolean) :
        CustomMarkdownRules.BlockQuoteState<ParseState> {
    ...
    class FooRule<S> : Rule<Any?, Node<Any?>, S>(Pattern.compile("^<Foo>")) {
        override fun parse(
            matcher: Matcher,
            parser: Parser<Any?, in Node<Any?>, S>,
            state: S
        ): ParseSpec<Any?, S> {
    ...
    class UserMentionRule<S> : Rule<RenderContext, UserNode, S>(Pattern.compile("^<(\\d+)>")) {
        override fun parse(
            matcher: Matcher,
            parser: Parser<RenderContext, in UserNode, S>,
            state: S
        ): ParseSpec<RenderContext, S> {
